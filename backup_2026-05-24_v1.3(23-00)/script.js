...
function renderTableRows() {
  ...
  function courseRow(course, subjectCell) {
    ...
    const borderStyle = isCustom && subjectColor ? `style="border-left:3px solid ${subjectColor}"` : '';
    return `<tr class="row--${course.subject||''} ${isDone ? 'row--done' : ''}" data-id="${course.id}" ${borderStyle}>
    ...
    <tr class="memo-row" id="memo-row-${course.id}">
      <td colspan="8">
